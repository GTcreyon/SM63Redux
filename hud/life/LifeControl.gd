...
onready var meter = $AnimatedSprite
...
func _process(_delta):
	...
	if(self.position.y < 40 && act):
		self.position.y += 8 #since it's in _process, it will keep adding until conditional stops being true
		if(self.position.y >= 40): #and then starts timer
			act = false
			del = true
	...
	meter.set_frame(player.life_meter_counter) #for the HUD with its respective frame
