...
func load_attributes():
	for i in Singleton.list_files_in_directory(properties_file_path + attribute_directory):
		if i == null: break
...
func load_bodytypes():
	for i in Singleton.list_files_in_directory(properties_file_path + bodytype_directory):
		if i == null: break
		var script = load(properties_file_path + attribute_directory + "/" + i)
