...
void fragment() {
	...
	if (col.r == 0.) {
		...
		outline_sum += sign(texture(viewport_texture, UV + vec2(0, pixel_scale.y)).r);
		outline_sum += sign(texture(viewport_texture, UV + vec2(0, pixel_scale.y * 2.)).r);
		outline_sum += sign(texture(viewport_texture, UV - vec2(0, pixel_scale.y)).r);
		outline_sum += sign(texture(viewport_texture, UV - vec2(0, pixel_scale.y * 2.)).r);
		outline_sum += sign(texture(viewport_texture, UV + vec2(pixel_scale.x, 0)).r);
		outline_sum += sign(texture(viewport_texture, UV + vec2(pixel_scale.x * 2., 0)).r);
		outline_sum += sign(texture(viewport_texture, UV - vec2(pixel_scale.x, 0)).r);
		outline_sum += sign(texture(viewport_texture, UV - vec2(pixel_scale.x * 2., 0)).r);
		if (outline_sum != 8.) {
			col = outline_color;
